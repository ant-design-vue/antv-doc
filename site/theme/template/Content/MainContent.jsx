...
const defaultPathName = 'docs/vue/introduce'

function getActiveMenuItem(props) {
  const children = props.params.children;
  let pathname = props.location.pathname
  pathname = pathname === '/'  || pathname === 'index-cn' ? fileNameToPath(defaultPathName) : pathname
  return (children && children.replace('-cn', '')) ||
    pathname.replace(/(^\/|-cn$)/g, '');
}

function getModuleData(props) {
  let pathname = props.location.pathname;
  if (pathname === '/' || pathname === 'index-cn') {
    pathname =  utils.isZhCN(pathname) ? `${defaultPathName}-cn` : defaultPathName
  }
  const moduleName = /^\/?components/.test(pathname) ?
          'components' : pathname.split('/').filter(item => item).slice(0, 2).join('/');
  const moduleData = moduleName === 'components' || moduleName === 'docs/vue' ||
          moduleName === 'changelog' || moduleName === 'changelog-cn' ?
          [...props.picked.components, ...props.picked['docs/vue'], ...props.picked.changelog] :
...
export default class MainContent extends React.Component {
  ...
  generateMenuItem(isTop, item) {
    ...
    if (url === defaultPathName || url === `${defaultPathName}-cn`) {
      url = '/'
    }
